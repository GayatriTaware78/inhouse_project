...
let quest = document.querySelector("#question-text"); // Make sure this is the correct selector for the question text element
...
if (score && prevScore !== null) { // Check if score element exists and prevScore is not null
...
quiz.addEventListener('submit', (e) => {
    ...
    if (evaluateAnswer(correctAnswer, ans.value)) {
        ...
        if (score) { // Check if score element exists
            score.textContent = `Score : ${prevScore}`;
        }
    } else {
...
async function initWebcam() {
    ...
    window.requestAnimationFrame(loop);
         ...
         console.error("Error initializing webcam and model:", error);
    } // Start sign language detection loop
}
async function loop() {
    webcam.update();
